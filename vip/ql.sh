...
DOCKER_IMG_NAME="registry.cn-hongkong.aliyuncs.com/shippoo/qinglong"
JD_PATH=""
SHELL_FOLDER=$(pwd)
CONTAINER_NAME=""
TAG="${ql}"
...
if [ "$access" != "2" ]; then
    if [ "$(grep -c "token" $CONFIG_PATH/auth.json)" != 0 ]; then
        ...
        docker exec -it $CONTAINER_NAME bash -c "$(curl -fsSL https://yanyu.ltd/https://raw.githubusercontent.com/wj2008/wjress/master/vip/1customCDN.sh)"
    else
